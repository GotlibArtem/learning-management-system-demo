...
@pytest.fixture
def provider(user, product, end_date, make_dt):
    return ProductAccessProvider(
        user=user,
        product=product,
        start_date=datetime(2030, 1, 1, 0, 0, 0, 0, tzinfo=msk),
        end_date=end_date,
        order_id="some-id",
...
def test_access_is_granted(provider, end_date, user, product, make_dt):
    provider()

    access = ProductAccess.objects.get()
    assert access.user == user
    assert access.product == product
    assert access.start_date == datetime(2030, 1, 1, 0, 0, 0, 0, tzinfo=msk)
    assert access.end_date == end_date
...
def test_update_existing_access_by_order_id(provider, product_access, end_date, user, product):
    provider()

    product_access.refresh_from_db()
    assert product_access.user == user
    assert product_access.product == product
    assert product_access.start_date == datetime(2030, 1, 1, 0, 0, 0, 0, tzinfo=msk)
    assert product_access.end_date == end_date
...
def test_empty_access_is_filled_in_in_any_case(provider, empty_access, user, product, end_date, make_dt):
    provider()

    empty_access.refresh_from_db()
    assert empty_access.user == user
    assert empty_access.product == product
    assert empty_access.start_date == datetime(2030, 1, 1, 0, 0, 0, 0, tzinfo=msk)
    assert empty_access.end_date == end_date
