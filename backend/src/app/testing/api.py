...
class ApiClient(DRFAPIClient):
    ...
    Client is available as two fixtures: `as_anon` and `as_user`. Use it like this:
        ...
        def test(as_anon):
            as_anon.get("/api/demo/healthchecks/db/")  # fetch endpoint anonymously


        def test_whoami(as_user, user):
            result = as_user.get("/api/demo/users/me/")  # fetch endpoint, authenticated as current user.

            assert result["id"] == user.pk  # fixture `as_user` always takes the `user` fixture


        def test_raw(as_user, user):
            raw = as_user.get("/api/demo/users/me/", as_response=True)  # get raw django HTTPResponse
    ...
    And use this fixture like this:
        ...
        def test_whoami(as, another_user):
            result = as(another_user).get("/api/demo/users/me/")  # your custom user to authenticate
